...
proc pogo::command {} {
    ...
    switch [lindex $text 0] {
        ...
        default {
            set idx [lsearch -index 0 $silph(listen) \
                    [list $userId $channelId $guildId]]
...
proc pogo::showcard {text} {
    ...
    if {$data != ""} {
        ...
        set silphacc [pogodb eval {
            SELECT account FROM pogo WHERE id = :id LIMIT 1
        }]
        ...
        if {$silphacc == ""} {
            ...
            if {$text in {me {}}} {
                ...
                set silph(listen) [list [list $userId $channelId $guildId] \
                        $username]
    ...
    switch $status {
        ok {
            ...
            if {[lindex [dict keys $silphdict] 0] == "error"} {
                set msg "No such user named $silphacc found on sil.ph"
                set msg [dict create content $msg]
            } else {
                ...
                foreach social $socials {
                    switch [dict get $social vendor] {
                        "Discord" {
                            set discord ":ballot_box_with_check: Connected to "
                            append discord "Discord: "
                            append discord [dict get $social username]
                        }
                        "Reddit" {
                            set reddit ":ballot_box_with_check: Connected to "
                            append reddit: "Reddit: [dict get $social username]"
                        }
                ...
                set playstyle "$style, working on $goal.\nActive around $home."
                append playstyle "\n\n$discord"
                set silphstats "**Joined:** $joined\n**Badges:** $badges\n"
                append silphstats "**Check-ins:** $checkins\n**Handshakes:** "
                append silphstats "$handshakes\n**Migrations:** $migrations"
                set gamestats "**Name:** $usrname\n**Team:** $team\n**Level:** "
                append gamestats "$level\n**Pokedex:** $pokedex\n**Avg "
                append gamestats "raids/week:** $raids"
                set footertext "Silph Road Travelers Card - ID$cardid - Last "
                append footertext "updated $modified"
                set icon "https://assets.thesilphroad.com/img/snoo_sr_icon.png"
        ...
        default {
            set msg "Error: Could not retrieve data from sil.ph. Status: "
            append msg $status
            set msg [dict create content $msg]
        }
    }
